...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public List<ScheduleResponse> getAllSchedule(String word) {
        ...
        if (word != null) {
            sql += " WHERE concat(d.first_name, '', d.last_name) LIKE '%" + word + "%'\n";
        }
        sql += " ORDER BY sdt.date ASC ";


        return template.query(sql, (resultSet, i) -> {
            Map<LocalTime, LocalTime> times = new HashMap<>();
            times.put(resultSet.getTime("timeFrom").toLocalTime(), resultSet.getTime("timeTo").toLocalTime());
